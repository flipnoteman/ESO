...
use psp_font::{render_font, test_font};
use core::{f32::consts::PI, ptr};
use psp::Align16;
use psp::sys::{
    self, ClearBuffer, CtrlButtons, DepthFunc, DisplayPixelFormat, FrontFaceDirection,
    GuContextType, GuPrimitive, GuState, GuSyncBehavior, GuSyncMode, MipmapLevel, ScePspFVector3,
    ShadingModel, TextureColorComponent, TextureEffect, TextureFilter, TexturePixelFormat,
    VertexType, sceGuBlendFunc, sceGuEnable,
};
...
fn init_Gu( allocator: Res<VramAllocator>) {
    unsafe {
        psp::enable_home_button();

        let fbp0 =
            allocator.0.alloc_texture_pixels(BUF_WIDTH, SCREEN_HEIGHT, TexturePixelFormat::Psm8888);
        let fbp1 =
            allocator.0.alloc_texture_pixels(BUF_WIDTH, SCREEN_HEIGHT, TexturePixelFormat::Psm8888);
        let zbp =
            allocator.0.alloc_texture_pixels(BUF_WIDTH, SCREEN_HEIGHT, TexturePixelFormat::Psm4444);
...
#[derive(Resource)]
pub struct VramAllocator(SimpleVramAllocator);
...
unsafe fn psp_main_inner() {
    ...
    world.insert_resource(VramAllocator(get_vram_allocator().unwrap()));
